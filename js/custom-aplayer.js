(() => {
  ...
  function initPlayer(APlayerConstructor) {
    const container = document.getElementById('aplayer');
    if (!container || window.aplayerInstance) return;

    window.aplayerInstance = new APlayerConstructor({
      container,
      fixed: false,
      autoplay: false,
      theme: '#fe7300',
  ...
  function startPlayer() {
    const ready = window.aplayerReady || (window.APlayer
      ? Promise.resolve(window.APlayer)
      : Promise.reject(new Error('APlayer loader is unavailable')));
    ready.then(initPlayer).catch(error => console.error('[aplayer]', error));
  }
